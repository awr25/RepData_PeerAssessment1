...
Ignoring missing values, we now calculate some summary statistics for the data, in particular:
- the total number of steps taken per day
- a histogram of the total number of steps taken each day
- the mean and median of the total number of steps taken per day

```{r summarisedata1, echo=TRUE}
...
We now make a time series plot to assess the step variation during the day by computing the mean step count over all days in each time interval.  The horizontal axis shows the 5 minute step interval and identify the time interval in the day where the step count is greatest.
...
We now consider the missing values by computing how many there are, and then imputing values for them as the mean number of steps for a particular time interval averaged over all days.A new dataset is created with these values filled in.
...
To assess the impact of filling in the missing values we recaluclate the summary statistics from before and plot a new histogram of the step count:

```{r filledinvaluessummary,echo=TRUE}
...
In this case, the imputation of missing values has increased the mean and median step count/day.

###Weekday vs Weekend activity

We can now make a comparison between the time series for weekday and weekend activity as follows:

```{r weekdayweekend,fig.width=8, fig.height=10,echo=TRUE}
...
We can see the increased number of steps on weekdays around the morning commute, and to a lesser extent, the evening commute. In contrast, the weekend step count is less spiked around these times of day.